const API_KEYS = {
    ...
    news: '899dc4f654794709853c0697a331d82e',
...
async function fetchNews() {
    ...
    const url = `https://newsapi.org/v2/everything?q=Schweiz&language=de&sortBy=publishedAt&apiKey=${API_KEYS.news}`; //
    try {
        const response = await fetch(url);
...
async function fetchApod() {
    ...
    try {
        ...
        // Setze das Bild als Hintergrund der Sektion
...
// Initiales Laden
fetchWeather('Zürich'); // Startet mit Zürich als Standard
fetchNews();
fetchApod();
setInterval(updateClock, 1000); // Startet die Uhr und aktualisiert sie jede Sekunde
updateClock(); // Zeigt die Uhr sofort an, ohne eine Sekunde zu warten